...
function PaymentGateway() {
  ...
  const { orderData } = location.state || {}; // Get order data passed from Checkout

  useEffect(() => {
    if (!orderData) {
        navigate('/'); // If no data, go home
        return;
    }
  ...
  const confirmOrder = async () => {
    ...
    try {
        ...
        // If successful, clear cart (optional logic) and go to Orders page
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center">
      <div className="bg-white p-10 rounded-2xl shadow-xl text-center max-w-md w-full border border-gray-100">
        ...
        <p className="text-gray-500 mb-6">Processing payment of <span className="font-bold text-gray-900">${orderData?.totalAmount}</span></p>
